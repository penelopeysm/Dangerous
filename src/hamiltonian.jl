...
e.g. ``I_{1z}`` in a 4-spin system is generated by ``op1(1, 4, SingleSpin.Z)``.
"""
...
e.g. ``I_{1z}I_{2x}`` in a 4-spin system is generated by ``op2(1, 2, 4, SingleSpin.Z, SingleSpin.X)``.
"""
...
function X(i, n)
    return op1(i, n, SingleSpin.X)
end

function Y(i, n)
    return op1(i, n, SingleSpin.Y)
end

function Z(i, n)
    return op1(i, n, SingleSpin.Z)
end

function XX(i, j, n)
    return op2(i, j, n, SingleSpin.X, SingleSpin.X)
end

function YY(i, j, n)
    return op2(i, j, n, SingleSpin.Y, SingleSpin.Y)
end

function ZZ(i, j, n)
    return op2(i, j, n, SingleSpin.Z, SingleSpin.Z)
end
...
function h_offset(sys)
    n = length(sys.nuclei)
    return sum([2π * sys.magnetic_field * sys.chemical_shifts[i] * Z(i, n) for i in 1:n])
end
...
function h_pulse(sys, nuc, ω1, φ)
    n = length(sys.nuclei)
    # Assume that pulses only apply to one nucleus
    indices = findall(==(nuc), sys.nuclei)
    return sum([op1(i, n, SingleSpin.h_pulse(ω1, φ)) for i in indices])
end
